...
public class LoggingSwitchFragment extends Fragment
    ...
    private static final long MINUTE = 1000; //1秒を基準に
//    private static final long MINUTE = 60 * 1000; //おそらく1分を基準にしていた。
    // 位置情報取得の間隔(ms)
    private static final long INTERVAL_MS = 10 * MINUTE; //10秒
    // 位置情報取得の最短間隔(ms)
    private static final long FASTEST_INTERVAL_MS = 5 * MINUTE; //5秒
    // 位置情報の更新を待つ最長間隔(ms)
    private static final long MAX_WAIT_TIME_MS = 60 * MINUTE; //1分
